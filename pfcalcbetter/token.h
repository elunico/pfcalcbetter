...
/// If the caller passes tok as NULL then use `token_free` to free the newly allocated returned token
/// If the caller passes memory it owns as tok then it is responsible to deal with **however** the caller must free `tok.value` which is the char data of the token
